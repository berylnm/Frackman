...
:People(IID_PLAYER,30,60,s,'F',right),m_squirt(5),m_gold(0),m_sonar(1)
{   setlife(10);}
...
:People(imageID,60,60,s,name,left),m_tick(max(0, 3 -getWorld()->getLevel()/4)),numStep(randInt(8,60)),perpendicular(0){}
...
bool Hardcorep::follow()
{
    int M=16+getWorld()->getLevel()*2;
    if (getWorld()->track(getX(), getY()) <= M)
    {
        setDirection(getWorld()->gettrace(getX(), getY()));
        int a=getX(),b=getY();
        if (movable(a, b, getDirection()))
        {
            moveTo(a,b);
...
{
    setVisible(true);
    setlife(max(100,300-10*getWorld()->getLevel()));
}
