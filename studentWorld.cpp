...
void StudentWorld::initialPath(Actor *a)
{
    
    for (int j=60; j>=0; j--)
    {
        for (int i=60; i>=0; i--)
        {   if (!existdirt(i, j) && !overlap(i, j))
            {
                path[i][j] = designpath(a, i, j);
            }
            else
                path[i][j] = Actor::none;
        }
...
Actor::Direction StudentWorld::designpath(Actor *a,int x, int y)
{   int x0=x, y0=y;
    if (a->movable(x0, y0, Actor::right) && path[x0][y0] != Actor::left)
        return Actor::right;
    if (a->movable(x0, y0, Actor::up) && path[x0][y0] != Actor::down)
        return Actor::up;
    if (a->movable(x0, y0, Actor::left) && path[x0][y0] != Actor::right)
        return Actor::left;
    if (a->movable(x0, y0, Actor::down)&& path[x0][y0] != Actor::up)
        return Actor::down;
    return Actor::none;
